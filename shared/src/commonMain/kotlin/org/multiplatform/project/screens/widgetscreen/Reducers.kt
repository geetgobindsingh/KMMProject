...
val reducer: Reducer<WidgetScreenState> = { state, action ->
    when (action) {
        ...
        is Actions.Timer.StartTimerAction -> timerReducer(state, action)
        is Actions.Timer.StopTimerAction -> timerReducer(state, action)
        else -> state
    }
}

fun timerReducer(state: WidgetScreenState, action: Any): WidgetScreenState = when(action) {
